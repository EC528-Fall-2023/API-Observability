...
const TopGraph = ({ data,title }) => {
  ...
  useEffect(() => {
    if (data && chartRef.current) {
      ...
      const width = 900;
      const height = 300;
      const margin = { top: 40, right: 10, bottom: 40, left: 60 }; // Adjusted margin to fit labels
      ...
      focus.append("text")
        ...
        function mousemove(event) {
           ...
           // Set the position of the focus group to the data point
    focus.attr("transform", `translate(${x(new Date(d.timestamp))},${y(d.value)})`);
    ...
    focus.select("text")
        ...
        .text(`${d.value.toFixed(3)} at ${d3.timeFormat("%B %d, %Y %I:%M:%S %p")(new Date(d.timestamp))}`); // Include time in the text
}
